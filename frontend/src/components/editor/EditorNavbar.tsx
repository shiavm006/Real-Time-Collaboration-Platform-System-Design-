      ...
      <div className="flex -space-x-2">
        {visible.map((uid, i) => {
          ...
          return (
            <Tooltip
              key={uid}
              text={`User ${uid.slice(0, 6)} ${isTyping ? "(typing...)" : ""}`}
            >
              <div
                className={`relative rounded-full transition-transform hover:scale-110 hover:z-10 ${isTyping ? "ring-2 ring-brand-500 ring-offset-1 ring-offset-surface" : ""}`}
              >
                <Avatar name={uid.slice(0, 6)} userId={uid} size="sm" />
              </div>
            </Tooltip>
          );
        })}
