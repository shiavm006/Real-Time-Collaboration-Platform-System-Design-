...
const DocIcon = () => (
  <svg className="w-8 h-8 text-brand-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.25" strokeLinecap="round" strokeLinejoin="round">
  ...
  if (!authLoading && !isAuthenticated) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-32 animate-fade-in">
        ...
        <h1 className="text-2xl font-semibold text-foreground tracking-tight mb-2">Welcome to CollabDoc</h1>
        <p className="text-secondary text-center max-w-sm mb-8 text-sm">
          Real-time collaborative document editing. Create, share, and work together seamlessly.
        </p>
            ...
            >
              <Link href={`/editor/${doc.id}`} className="block p-5">
                ...
                <p className="text-xs text-muted">
                  {doc.updated_at ? new Date(doc.updated_at).toLocaleDateString(undefined, {
