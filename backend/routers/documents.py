...
async def get_permissions(doc_id: str, user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    doc = await DocumentService.get(db, uuid.UUID(doc_id))
    if not doc:
        raise HTTPException(status_code=404, detail="Document not found")
    if not await PermissionService.can_view(db, doc.id, user.id):
...
async def restore_version(doc_id: str, version_id: str, user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    doc = await DocumentService.get(db, uuid.UUID(doc_id))
    if not doc:
        raise HTTPException(status_code=404, detail="Document not found")
    role = await PermissionService.get_role(db, doc.id, user.id)
